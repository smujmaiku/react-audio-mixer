...
export function getNote(list: NoteList, hz: number): [string, number] {
	const min = Object.values(list).slice(0, 2).reduce((a, b) => 2 * a - b);
	const max = Object.values(list).slice(-2).reduce((a, b) => 2 * b - a);
...
export function combineByNotes(noteList: NoteList, tones: AnalyserToneT[]): AnalyserToneT[] {
